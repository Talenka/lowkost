...
var Physijs = (function() {
  ...
  convertWorldPositionToObject = function(position, object) {
    ...
    // Invert rotation matrix in order to "unrotate"
    // a point back to object space
   ...
   */
  Physijs.HingeConstraint.prototype.setLimits = function(low, high, bias_factor,
                                                         relaxation_factor) {
    this.scene.execute('hinge_setLimits',
        {constraint: this.id, low: low, high: high, bias_factor: bias_factor,
          relaxation_factor: relaxation_factor});
  };

  Physijs.HingeConstraint.prototype.enableAngularMotor =
      function(velocity, acceleration) {
    this.scene.execute('hinge_enableAngularMotor',
        {constraint: this.id, velocity: velocity, acceleration: acceleration});
  };

  Physijs.HingeConstraint.prototype.disableMotor =
      function(velocity, acceleration) {
  ...
  Physijs.SliderConstraint.prototype.setLimits = function(lin_lower,
                                                          lin_upper,
                                                          ang_lower,
                                                          ang_upper) {
    this.scene.execute('slider_setLimits',
        {constraint: this.id, lin_lower: lin_lower, lin_upper: lin_upper,
          ang_lower: ang_lower, ang_upper: ang_upper});
  };
  Physijs.SliderConstraint.prototype.setRestitution = function(linear,
                                                               angular) {
    this.scene.execute(
  ...
  Physijs.SliderConstraint.prototype.enableLinearMotor =
      function(velocity, acceleration) {
  ...
  Physijs.SliderConstraint.prototype.enableAngularMotor =
      function(velocity, acceleration) {
  ...
  Physijs.ConeTwistConstraint = function(objecta, objectb, position) {
    ...
    this.objectb = objectb._physijs.id;
    this.positionb = convertWorldPositionToObject(position, objectb).clone();
    this.axisa = { x: objecta.rotation.x,
                   y: objecta.rotation.y,
                   z: objecta.rotation.z};
    this.axisb = { x: objectb.rotation.x,
  ...
  Physijs.ConeTwistConstraint.prototype.setLimit = function(x, y, z) {
    this.scene.execute('conetwist_setLimit',
        {constraint: this.id, x: x, y: y, z: z});
  };
  Physijs.ConeTwistConstraint.prototype.enableMotor = function() {
    this.scene.execute('conetwist_enableMotor', {constraint: this.id});
  };
  Physijs.ConeTwistConstraint.prototype.setMaxMotorImpulse =
      function(max_impulse) {
    this.scene.execute('conetwist_setMaxMotorImpulse',
        {constraint: this.id, max_impulse: max_impulse});
  };
  Physijs.ConeTwistConstraint.prototype.setMotorTarget = function(target) {
    if (target instanceof THREE.Vector3) {
      throw 'Wait for Three.js r50 to setMotorTarget from Vector3 - ' +
          'use Matrix4 or Quaternion instead';
    ...
    this.scene.execute('conetwist_setMotorTarget',
        { constraint: this.id, x: target.x, y: target.y,
  ...
  Physijs.DOFConstraint.prototype.setLinearLowerLimit = function(limit) {
    this.scene.execute('dof_setLinearLowerLimit',
        {constraint: this.id, x: limit.x, y: limit.y, z: limit.z});
  };
  ...
  Physijs.DOFConstraint.prototype.enableAngularMotor = function(which) {
    this.scene.execute('dof_enableAngularMotor',
        {constraint: this.id, which: which});
  };
  Physijs.DOFConstraint.prototype.configureAngularMotor = function(which,
                                                                   low_angle,
                                                                   high_angle,
                                                                   velocity,
                                                                   max_force) {
    this.scene.execute('dof_configureAngularMotor',
        { constraint: this.id, which: which, low_angle: low_angle,
          high_angle: high_angle, velocity: velocity, max_force: max_force});
  };
  ...
  Physijs.Scene = function(params) {
    ...
    this._worker.transferableMessage = this._worker.webkitPostMessage ||
  ...
  Physijs.Scene.prototype._updateCollisions = function(data) {
    /**
     * #TODO
     * This is probably the worst way ever to handle collisions.
     * The inherent evilness is a residual
     * effect from the previous version's evilness which mutated when
     * switching to transferable objects.
     *
        ...
        for (j = 0; j < collisions[object._physijs.id].length; j++) {
          ...
          if (object2) {
            if (object._physijs.touches.indexOf(object2._physijs.id) === -1) {
              ...
              _temp_vector3_1.subVectors(object.getLinearVelocity(),
                                         object2.getLinearVelocity());
              _temp1 = _temp_vector3_1.clone();

              _temp_vector3_1.subVectors(object.getAngularVelocity(),
                                         object2.getAngularVelocity());
  ...
  Physijs.Scene.prototype.add = function(object) {
    ...
    if (object._physijs) {
      ...
      if (object instanceof Physijs.Vehicle) {
        ...
        object._physijs.position = {x: object.position.x,
                                     y: object.position.y,
                                     z: object.position.z};
        ...
        object._physijs.rotation = {x: object.quaternion.x,
  ...
  Physijs.Scene.prototype.simulate = function(timeStep, maxSubSteps) {
    ...
    if (_is_simulating) return false;
    ...
    for (object_id in this._objects) {
      ...
      if (object.__dirtyPosition || object.__dirtyRotation) {
        ...
        if (object.__dirtyPosition) {
          update.pos = {x: object.position.x,
                         y: object.position.y,
                         z: object.position.z};
        ...
        if (object.__dirtyRotation) {
          ...
          update.quat = {x: object.quaternion.x,
                          y: object.quaternion.y,
                          z: object.quaternion.z,
                          w: object.quaternion.w};
  ...
  Physijs.Mesh.prototype.applyCentralImpulse = function(force) {
    if (this.world)
      this.world.execute('applyCentralImpulse',
          {id: this._physijs.id, x: force.x, y: force.y, z: force.z});
  };
  ...
  Physijs.Mesh.prototype.applyImpulse = function(force, offset) {
    if (this.world)
      this.world.execute('applyImpulse',
          {id: this._physijs.id, impulse_x: force.x, impulse_y: force.y,
            impulse_z: force.z, x: offset.x, y: offset.y, z: offset.z});
  };
  ...
  Physijs.Mesh.prototype.applyForce = function(force, offset) {
    if (this.world)
      this.world.execute('applyForce',
          {id: this._physijs.id, force_x: force.x, force_y: force.y,
            force_z: force.z, x: offset.x, y: offset.y, z: offset.z });
  };
  ...
  Physijs.Mesh.prototype.setAngularVelocity = function(velocity) {
    if (this.world)
      this.world.execute('setAngularVelocity',
          {id: this._physijs.id, x: velocity.x, y: velocity.y, z: velocity.z});
  };
  ...
  Physijs.Mesh.prototype.setAngularFactor = function(factor) {
    if (this.world)
      this.world.execute('setAngularFactor',
          {id: this._physijs.id, x: factor.x, y: factor.y, z: factor.z});
  };
  ...
  Physijs.Mesh.prototype.setDamping = function(linear, angular) {
    if (this.world)
      this.world.execute('setDamping',
          {id: this._physijs.id, linear: linear, angular: angular});
  ...
  Physijs.Mesh.prototype.setCcdMotionThreshold = function(threshold) {
    if (this.world)
      this.world.execute('setCcdMotionThreshold',
          {id: this._physijs.id, threshold: threshold});
  ...
  Physijs.Mesh.prototype.setCcdSweptSphereRadius = function(radius) {
    if (this.world)
      this.world.execute('setCcdSweptSphereRadius',
          {id: this._physijs.id, radius: radius});
  };
  ...
  Physijs.HeightfieldMesh = function(geometry, material, mass, xdiv, ydiv) {
    ...
    this._physijs.xsize = geometry.boundingBox.max.x -
                          geometry.boundingBox.min.x;
    this._physijs.ysize = geometry.boundingBox.max.y -
                          geometry.boundingBox.min.y;

    this._physijs.xpts = (typeof xdiv === 'undefined') ?
                         Math.sqrt(geometry.vertices.length) : xdiv + 1;
    this._physijs.ypts = (typeof ydiv === 'undefined') ?
                         Math.sqrt(geometry.vertices.length) : ydiv + 1;

    // note - this assumes our plane geometry is square,
    // unless we pass in specific xdiv and ydiv
    this._physijs.absMaxHeight = Math.max(geometry.boundingBox.max.z,
    ...
    for (var i = 0; i < geometry.vertices.length; i++) {
      ...
      b = Math.round((i / this._physijs.xpts) -
          ((i % this._physijs.xpts) / this._physijs.xpts));

      points[i] = geometry.vertices[a + ((this._physijs.ypts - b - 1) *
                                    this._physijs.ypts)].z;
  ...
  Physijs.BoxMesh = function(geometry, material, mass) {
    ...
    this._physijs.mass = (typeof mass === 'undefined') ?
  ...
  Physijs.SphereMesh = function(geometry, material, mass) {
    ...
    this._physijs.mass = (typeof mass === 'undefined') ?
                         (4 / 3) * Math.PI * Math.pow(this._physijs.radius, 3) :
                         mass;
  };
  ...
  Physijs.Vehicle.prototype.addWheel = function(wheel_geometry,
                                                wheel_material,
                                                connection_point,
                                                wheel_direction,
                                                wheel_axle,
    ...
    wheel.position.copy(wheel_direction)
                  .multiplyScalar(suspension_rest_length / 100)
                  .add(connection_point);
  ...
  Physijs.Vehicle.prototype.setSteering = function(amount, wheel) {
    ...
    if (wheel !== undefined && this.wheels[wheel] !== undefined)
      this.world.execute('setSteering',
          {id: this._physijs.id, wheel: wheel, steering: amount});

    else if (this.wheels.length > 0) {
      for (var i = 0; i < this.wheels.length; i++)
        this.world.execute('setSteering',
            {id: this._physijs.id, wheel: i, steering: amount});
    }
  };
  ...
  Physijs.Vehicle.prototype.setBrake = function(amount, wheel) {
    if (wheel !== undefined && this.wheels[wheel] !== undefined)
      this.world.execute('setBrake',
          {id: this._physijs.id, wheel: wheel, brake: amount});

    else if (this.wheels.length > 0) {
      for (var i = 0; i < this.wheels.length; i++)
        this.world.execute('setBrake',
            {id: this._physijs.id, wheel: i, brake: amount});
    }
  };
  ...
  Physijs.Vehicle.prototype.applyEngineForce = function(amount, wheel) {
    ...
    if (wheel !== undefined && this.wheels[wheel] !== undefined)
      this.world.execute('applyEngineForce',
          {id: this._physijs.id, wheel: wheel, force: amount});

    else if (this.wheels.length > 0) {

      for (var i = 0; i < this.wheels.length; i++)
        this.world.execute('applyEngineForce',
            {id: this._physijs.id, wheel: i, force: amount});

    }
  };

  // Physijs.VehicleTuning
  Physijs.VehicleTuning = function(suspension_stiffness,
    ...
    this.suspension_stiffness = (suspension_stiffness !== undefined) ?
                                suspension_stiffness : 5.88;

    this.suspension_compression = (suspension_compression !== undefined) ?
                                  suspension_compression : 0.83;

    this.suspension_damping = (suspension_damping !== undefined) ?
                              suspension_damping : 0.88;

    this.max_suspension_travel = (max_suspension_travel !== undefined) ?
                                 max_suspension_travel : 500;

    this.friction_slip = (friction_slip !== undefined) ? friction_slip : 10.5;

    this.max_suspension_force = (max_suspension_force !== undefined) ?
