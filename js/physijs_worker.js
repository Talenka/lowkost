...
var getShapeFromCache;
var setShapeCache;
var createShape;
var reportWorld;
var reportVehicles;
var reportCollisions;
...
var rateLimit;
var last_simulation_time;
var last_simulation_duration = 0;
var world;
var transform;
var _vec3_1;
var _vec3_2;
 ...
 * @return {Object} The created shape.
 */
createShape = function(description) {
  ...
  switch (description.type) {
    case 'plane':
      cache_key = 'plane_' + description.normal.x + '_' +
                  description.normal.y + '_' + description.normal.z;
    ...
    case 'box':
      cache_key = 'box_' + description.width + '_' +
                  description.height + '_' + description.depth;
    ...
    case 'cylinder':
      cache_key = 'cylinder_' + description.width + '_' +
                  description.height + '_' + description.depth;
    ...
    case 'capsule':
      ...
      if ((shape = getShapeFromCache(cache_key)) === null) {
        ...
        shape = new Ammo.btCapsuleShape(description.radius,
    ...
    case 'heightfield':
      ...
      var ptr = Ammo.allocate(4 * description.xpts * description.ypts,
                              'float', Ammo.ALLOC_NORMAL);
      ...
      shape = new Ammo.btHeightfieldTerrainShape(
          description.xpts, description.ypts, ptr, 1,
          -description.absMaxHeight, description.absMaxHeight, 2, 0, false);
...
public_functions.init = function(params) {
  ...
  // Transferable messages are supported, take advantage of them w/ TypedArrays
  if (SUPPORT_TRANSFERABLE) {
    // message id + # of objects to report + chunk size * # of values/object
    worldreport = new Float32Array(2 + REPORT_CHUNKSIZE *
                                   WORLDREPORT_ITEMSIZE);

    // message id + # of collisions to report + chunk size * # of values/object
    collisionreport = new Float32Array(2 + REPORT_CHUNKSIZE *
                                       COLLISIONREPORT_ITEMSIZE);

    // message id + # of vehicles to report + chunk size * # of values/object
    vehiclereport = new Float32Array(2 + REPORT_CHUNKSIZE *
                                     VEHICLEREPORT_ITEMSIZE);

    // message id + # of constraints to report + chunk size * # of values/object
    constraintreport = new Float32Array(2 + REPORT_CHUNKSIZE *
  ...
  world = new Ammo.btDiscreteDynamicsWorld(dispatcher, broadphase,
...
public_functions.addObject = function(description) {
  ...
  // #TODO: btDefaultMotionState supports center of
  // mass offset as second argument - implement
  motionState = new Ammo.btDefaultMotionState(_transform);
  rbInfo = new Ammo.btRigidBodyConstructionInfo(description.mass, motionState,
                                                shape, _vec3_1);
...
public_functions.addVehicle = function(description) {
  ...
  vehicle_tuning.set_m_suspensionCompression(
  ...
  vehicle =
      new Ammo.btRaycastVehicle(vehicle_tuning, _objects[description.rigidBody],
...
public_functions.addWheel = function(description) {
  if (_vehicles[description.id] !== undefined) {
    ...
    if (description.tuning !== undefined) {
      ...
      tuning.set_m_suspensionCompression(
          description.tuning.suspension_compression);

      tuning.set_m_suspensionDamping(description.tuning.suspension_damping);

      tuning.set_m_maxSuspensionTravelCm(
    ...
    _vehicles[description.id].addWheel(
        ...
        tuning, description.is_front_wheel);
  }

  _num_wheels++;

  if (SUPPORT_TRANSFERABLE) {
    // message id & (# of objects to report * # of values per object)
    vehiclereport = new Float32Array(1 + _num_wheels * VEHICLEREPORT_ITEMSIZE);
    vehiclereport[0] = MESSAGE_TYPES.VEHICLEREPORT;
  } else {
...
public_functions.updateMass = function(details) {
  ...
  // Per http://www.bulletphysics.org/Bullet/phpBB3/
  // viewtopic.php?p=&f=9&t=3663#p13816
...
public_functions.addConstraint = function(details) {
  ...
  switch (details.type) {
    ...
    case 'point':
      if (details.objectb === undefined) {
        ...
        constraint = new Ammo.btPoint2PointConstraint(
            _objects[details.objecta], _vec3_1);
      } else {
        ...
        constraint = new Ammo.btPoint2PointConstraint(
            _objects[details.objecta], _objects[details.objectb],
            _vec3_1, _vec3_2);
      }
      break;
    ...
    case 'hinge':
      if (details.objectb === undefined) {
        ...
        constraint = new Ammo.btHingeConstraint(
            _objects[details.objecta], _vec3_1, _vec3_2);
        ...
        constraint = new Ammo.btHingeConstraint(
            _objects[details.objecta], _objects[details.objectb],
            _vec3_1, _vec3_2, _vec3_3, _vec3_3);
      }
      break;
    ...
    case 'slider':
      ...
      if (details.objectb) {
        ...
        constraint = new Ammo.btSliderConstraint(
            _objects[details.objecta], transforma, true);
      }
    ...
    case 'conetwist':
      ...
      var rotation = transforma.getRotation();
      rotation.setEulerZYX(-details.axisa.z,
                           -details.axisa.y,
                           -details.axisa.x);
      transforma.setRotation(rotation);

      rotation = transformb.getRotation();
      rotation.setEulerZYX(-details.axisb.z,
                           -details.axisb.y,
                           -details.axisb.x);
      transformb.setRotation(rotation);
    ...
    case 'dof':
      ...
      else constraint = new Ammo.btGeneric6DofConstraint(
                                       _objects[details.objecta], transforma);
  ...
  if (SUPPORT_TRANSFERABLE) {
    // message id & (# of objects to report * # of values per object)
    constraintreport = new Float32Array(1 + _num_constraints *
                                        CONSTRAINTREPORT_ITEMSIZE);
...
/**
 * @param {Object} details Constraint parameters.
 */
...
public_functions.simulate = function simulate(params) {
  if (world) {
    ...
    // If maxSubSteps is not defined, keep the simulation fully up to date
    params.maxSubSteps = params.maxSubSteps ||
                         Math.ceil(params.timeStep / fixedTimeStep);
...
/**
 * @param {Object} params Motor's parameters.
 */
public_functions.hinge_setLimits = function(params) {
  _constraints[params.constraint].setLimit(params.low, params.high, 0,
                                           params.bias_factor,
...
public_functions.conetwist_setLimit = function(params) {
  // Attention : ZYX order
  _constraints[params.constraint].setLimit(params.z, params.y, params.x);
};
...
/**
 * @param {Object} params constraint's parameters.
 */
...
public_functions.dof_setLinearUpperLimit = function(params) {
  ...
  if (constraint.getRigidBodyB()) constraint.getRigidBodyB().activate();
};
...
reportWorld = function() {
  ...
  if (SUPPORT_TRANSFERABLE) {
    if (worldreport.length < 2 + _num_objects * WORLDREPORT_ITEMSIZE) {
      worldreport = new Float32Array(
          ...
          (Math.ceil(_num_objects / REPORT_CHUNKSIZE) * REPORT_CHUNKSIZE) *
          WORLDREPORT_ITEMSIZE // # of values needed * item size
          );
      worldreport[0] = MESSAGE_TYPES.WORLDREPORT;
    }
  }
  ...
  for (index in _objects) {
    if (_objects.hasOwnProperty(index)) {
      ...
      // #TODO: we can't use center of mass transform when center of mass
      // can change, but getMotionState().getWorldTransform() screws up on
  ...
  if (SUPPORT_TRANSFERABLE) {
    if (collisionreport.length < 2 + num * COLLISIONREPORT_ITEMSIZE) {
      collisionreport = new Float32Array(
          2 + // message id & # objects in report
          (Math.ceil(_num_objects / REPORT_CHUNKSIZE) * REPORT_CHUNKSIZE) *
          COLLISIONREPORT_ITEMSIZE // # of values needed * item size
          );
      collisionreport[0] = MESSAGE_TYPES.COLLISIONREPORT;
    }
  }
...
reportVehicles = function() {
  ...
  if (SUPPORT_TRANSFERABLE) {
    if (vehiclereport.length < 2 + _num_wheels * VEHICLEREPORT_ITEMSIZE) {
      vehiclereport = new Float32Array(
          2 + // message id & # objects in report
          (Math.ceil(_num_wheels / REPORT_CHUNKSIZE) * REPORT_CHUNKSIZE) *
          VEHICLEREPORT_ITEMSIZE // # of values needed * item size
          );
      vehiclereport[0] = MESSAGE_TYPES.VEHICLEREPORT;
    }
  }
  ...
  if (j !== 0) {
    ...
    else transferableMessage(vehiclereport);
  }
};
...
reportConstraints = function() {
  ...
  if (SUPPORT_TRANSFERABLE) {
    if (constraintreport.length < 2 +
        _num_constraints * CONSTRAINTREPORT_ITEMSIZE) {

      constraintreport = new Float32Array(
          2 + // message id & # objects in report
          (Math.ceil(_num_constraints / REPORT_CHUNKSIZE) * REPORT_CHUNKSIZE) *
          CONSTRAINTREPORT_ITEMSIZE // # of values needed * item size
          );
      constraintreport[0] = MESSAGE_TYPES.CONSTRAINTREPORT;
    }
  }
  ...
  if (i !== 0) {
    ...
    else transferableMessage(constraintreport);
  }
...
self.onmessage = function(event) {
  ...
  if (event.data.cmd && public_functions[event.data.cmd]) {
    //if (event.data.params.id !== undefined &&
    //    _objects[event.data.params.id] === undefined &&
    //    event.data.cmd !== 'addObject' &&
    //    event.data.cmd !== 'registerMaterial') return;
